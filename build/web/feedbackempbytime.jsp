<%-- 
    Document   : feedbackempbytime
    Created on : Jul 31, 2022, 7:15:02 PM
            ...
            <main>
                ...
                <div class="table-data">
                    <div class="order">
                        ...
                        <div>
                            <form action="MainController" method="POST">
                                <label for="commentTime">Lọc feedback theo thời gian:</label>
                        ...
                        <!--select by -->
                        <c:if test="${LIST_COMMENT_BY_TIME != null}">
                            <table>
                                <thead>
                                    <tr>
                                        <th>No.</th>
                                ...
                                <tbody>
                                    <c:forEach var="listCommentByTime" items="${LIST_COMMENT_BY_TIME}" varStatus="loop">
                                    <form action="MainController" method="POST">
                                        <tr>
                                        <input type="hidden" value="${listCommentByTime.commentID}" name="commentID"/> 
                                        <td>${loop.count}</td>
                                        <td>
                                            <p style="font-weight: 600; color: #002060;">${listCommentByTime.name}</p>
                                        </td>
                                        <td>${listCommentByTime.cmtTime}</td>
                                        <td style="font-weight: 500; color: #2eb7e5;"><span>${listCommentByTime.contentCmt}</span></td>
                                        <td><span class="status process" style="color: black;">${listCommentByTime.rate}</span></td>
                                        <td><span class="status pending">${listCommentByTime.status}</span></td>
                                            <c:if test="${listCommentByTime.doctorID == 1}">
                                            <td><span>Calvin Carlo</span></td>
                                        </c:if>
                                        <c:if test="${listCommentByTime.doctorID == 2}">
                                            <td><span>Cristino Murphy</span></td>
                                        </c:if>
                                        <c:if test="${listCommentByTime.doctorID == 3}">
                                            <td><span>Alia Reddy</span></td>
                                        </c:if>
                                        <c:if test="${listCommentByTime.doctorID == 4}">
                                            <td><span>Toni Kovar</span></td>
                                        </c:if>
                                        <c:if test="${listCommentByTime.doctorID == 5}">
                                            <td><span>Jessica McFarlane</span></td>
                                        </c:if>
                                        <c:if test="${listCommentByTime.doctorID == 6}">
                                            <td><span>Elsie Sherman</span></td>
                                        </c:if>
                                        <c:if test="${listCommentByTime.doctorID == 7}">
                                            <td><span>Bertha Magers</span></td>
                                        </c:if>
                                        <c:if test="${listCommentByTime.doctorID == 8}">
