            ...
            <main>
                ...
                <div class="table-data">
                    <div class="order">
                        ...
                        <!--select by odctor-->
                        <c:if test="${LIST_UNAVAILABLE_COMMENT != null}">
                            <table>
                                <thead>
                                    <tr>
                                        <th>No.</th>
                                ...
                                <tbody>
                                    <c:forEach var="listUnavaiComment" items="${LIST_UNAVAILABLE_COMMENT}" varStatus="loop">
                                    <form action="MainController" method="POST">
                                        <tr>
                                        <input type="hidden" value="${listUnavaiComment.commentID}" name="commentID"/> 
                                        <td>${loop.count}</td>
                                        <td>
                                            <p style="font-weight: 600; color: #002060;">${listUnavaiComment.name}</p>
                                        </td>
                                        <td>${listUnavaiComment.cmtTime}</td>
                                        <td style="font-weight: 500; color: #2eb7e5;"><span>${listUnavaiComment.contentCmt}</span></td>
                                        <td><span class="status process" style="color: black;">${listUnavaiComment.rate}</span></td>
                                        <td><span class="status pending">${listUnavaiComment.status}</span></td>
                                            <c:if test="${listUnavaiComment.doctorID == 1}">
                                            <td><span>Calvin Carlo</span></td>
                                        </c:if>
                                        <c:if test="${listUnavaiComment.doctorID == 2}">
                                            <td><span>Cristino Murphy</span></td>
                                        </c:if>
                                        <c:if test="${listUnavaiComment.doctorID == 3}">
                                            <td><span>Alia Reddy</span></td>
                                        </c:if>
                                        <c:if test="${listUnavaiComment.doctorID == 4}">
                                            <td><span>Toni Kovar</span></td>
                                        </c:if>
                                        <c:if test="${listUnavaiComment.doctorID == 5}">
                                            <td><span>Jessica McFarlane</span></td>
                                        </c:if>
                                        <c:if test="${listUnavaiComment.doctorID == 6}">
                                            <td><span>Elsie Sherman</span></td>
                                        </c:if>
                                        <c:if test="${listUnavaiComment.doctorID == 7}">
                                            <td><span>Bertha Magers</span></td>
                                        </c:if>
                                        <c:if test="${listUnavaiComment.doctorID == 8}">
