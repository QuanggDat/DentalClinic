            ...
            <main>
                ...
                <div class="table-data">
                    <div class="order">
                        ...
                        <c:if test="${LIST_DELETED_COMMENT_BY_DOCTORID != null}">
                            <table>
                                <thead>
                                    <tr>
                                        <th>No.</th>
                                ...
                                <tbody>
                                    <c:forEach var="listDeletedCommentByDoctorID" items="${LIST_DELETED_COMMENT_BY_DOCTORID}" varStatus="loop">
                                    <form action="MainController" method="POST">
                                        <tr>
                                        <input type="hidden" value="${listDeletedCommentByDoctorID.commentID}" name="commentID"/> 
                                        <td>${loop.count}</td>
                                        <td>
                                            <p style="font-weight: 600; color: #113355;">${listDeletedCommentByDoctorID.name}</p>
                                        </td>
                                        <td>${listDeletedCommentByDoctorID.cmtTime}</td>
                                        <td style="font-weight: 500; color: #2eb7e5;"><span>${listDeletedCommentByDoctorID.contentCmt}</span></td>
                                        <td><span class="status process" style="color: black;">${listDeletedCommentByDoctorID.rate}</span></td>
                                        <td><span class="status pending">${listDeletedCommentByDoctorID.status}</span></td>
                                            <c:if test="${listDeletedCommentByDoctorID.doctorID == 1}">
                                            <td><span>Calvin Carlo</span></td>
                                        </c:if>
                                        <c:if test="${listDeletedCommentByDoctorID.doctorID == 2}">
                                            <td><span>Cristino Murphy</span></td>
                                        </c:if>
                                        <c:if test="${listDeletedCommentByDoctorID.doctorID == 3}">
                                            <td><span>Alia Reddy</span></td>
                                        </c:if>
                                        <c:if test="${listDeletedCommentByDoctorID.doctorID == 4}">
                                            <td><span>Toni Kovar</span></td>
                                        </c:if>
                                        <c:if test="${listDeletedCommentByDoctorID.doctorID == 5}">
                                            <td><span>Jessica McFarlane</span></td>
                                        </c:if>
                                        <c:if test="${listDeletedCommentByDoctorID.doctorID == 6}">
                                            <td><span>Elsie Sherman</span></td>
                                        </c:if>
                                        <c:if test="${listDeletedCommentByDoctorID.doctorID == 7}">
                                            <td><span>Bertha Magers</span></td>
                                        </c:if>
                                        <c:if test="${listDeletedCommentByDoctorID.doctorID == 8}">
